...
function img($image,$username,$extension){
 ...
 if ($extension=='jpg'|| $extension=='jpeg') {
  ...
  if(imagejpeg($new_image,"upload/$username.jpg")){
	   return true;
  }else{
	   return false;
  }
...
function all_stories(){
  ...
  while ($story = mysqli_fetch_assoc($story_set)) {
    ...
    $output .= "<a class=\"media-left\" href=\"profile.php?user=$story[user_id]&username=$story[user_id] \"><img class=\"blogger-img \" src=\"upload/{$story['user_id']}.jpg\" ></img><br>{$story['user_id']}</a>";
    $output .= "<div class=\"media-body \">";
    $output .=  $story['text'];
    $output .= "<h6>" .$story['date']."</h6>";
    $output .= "</h4><br/>";
    $output .= "</div></div>";
    }
      mysqli_free_result($story_set);
